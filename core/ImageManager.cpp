...
ImageManager::ImageManager(IResourceManager *resourceManager) :
...
IImage *ImageManager::loadImage(/*const std::string&*/const char* filename)
{
	...
	m_resourceManager->addResource(pImage);
	return pImage;
}

IImage *ImageManager::getByName(const std::string& name) const
{
	return reinterpret_cast<IImage*>(m_resourceManager->getByName(name));
}

IResourceManager *ImageManager::resourceManager() const
{
