	...
	for (uint32 i = 0; i < indices.size(); i += 3) {
		...
		case ClassifyPlane_Intersect: {
			...
			if (numCuts == 2) {
				...
				uint32 cutIdx[2] = { idxs[(edgeIdx+0)%3], idxs[(edgeIdx+1)%3] }; // индексы вершин треугольника, ребро между которыми, нужно разрезать.
				...
				pI[1] = cutIdx[1];
				pI[2] = vertPlaneIdx;
				// Тут записать индексы в leftIndices, rightIndices!!!
			}
