...
void GLFrameBuffer::attachColorBuffer(const uint32 slot, const IGPURenderBuffer *pRenderBuffer)
{
	...
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+slot, GL_RENDERBUFFER, static_cast<const GLRenderBuffer*>(pRenderBuffer)->GLid());
}
...
void GLFrameBuffer::attachColorBuffer(const uint32 slot, const IGPUTexture *pTexture, const uint32 layer)
{
	...
	case IGPUTexture::IGPUTextureType_1D: {
		glFramebufferTexture1D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+slot, GL_TEXTURE_1D, texGLid, 0);
		break;
	}
	case IGPUTexture::IGPUTextureType_2D: {
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+slot, GL_TEXTURE_2D, texGLid, 0);
		break;
	}
	case IGPUTexture::IGPUTextureType_Rectangle: {
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+slot, GL_TEXTURE_RECTANGLE, texGLid, 0);
		break;
	}
	case IGPUTexture::IGPUTextureType_CubeMap: {
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+slot, GL_TEXTURE_CUBE_MAP_POSITIVE_X+layer, texGLid, 0);
		break;
	}
	case IGPUTexture::IGPUTextureType_3D: {
		glFramebufferTexture3D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+slot, GL_TEXTURE_3D, texGLid, 0, layer);
		break;
	}
	case IGPUTexture::IGPUTextureType_1DArray:
	case IGPUTexture::IGPUTextureType_2DArray:{
		glFramebufferTextureLayer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+slot, texGLid, 0, layer);
		break;
	}
	}
}
