...
struct Mesh {
	...
	float *pVertexData; // массив с вершинными атрибутами. –азмер = numVertices * vertexStride * sizeof(float)
	uint32 *pIndexData; // массив с индексами. –азмер = numIndices * sizeof(uint32)
	uint32 numVertices; // количество вершин
	uint32 numIndices; // количество индексов
	PrimitiveFormat primitiveFormat; // тип примитивов
	std::map<AttributeType, std::pair<uint16, uint16> > attributes; // отображение типа атрибута на пару (размер, смещение). –азмер и смещение в штуках float
